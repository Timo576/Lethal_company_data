...
public class HUDManager : NetworkBehaviour
{
    ...
    for (int index1 = 0; index1 < StartOfRound.Instance.allPlayerScripts.Length; ++index1)
    {
      ...
      else
      {
        ...
        if (!GameNetworkManager.Instance.disableSteam)
          this.FillSpectateBoxImageWithSteamProfile(gameObject.GetComponent<RawImage>(), playerScript);
      }
    }
  }

  private async void FillSpectateBoxImageWithSteamProfile(RawImage image, PlayerControllerB player)
  {
  ...
  private void SetScreenFilters()
  {
    ...
    if ((double) this.flashbangScreenFilter.weight > 0.0)
      this.flashbangScreenFilter.weight -= Time.deltaTime * 0.16f;
  ...
  public void FillEndGameStats(EndOfGameStats stats)
  {
    ...
    this.statsUIElements.quotaNumerator.text = RoundManager.Instance.scrapCollectedInLevel.ToString();
