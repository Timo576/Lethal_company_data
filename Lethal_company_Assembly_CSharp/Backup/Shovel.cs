...
public class Shovel : GrabbableObject
{
  ...
  public void HitShovel(bool cancel = false)
  {
    ...
    else
    {
      ...
      Debug.Log((object) ("PLAYER HIT WITH SHOVEL! who is sending this log: " + GameNetworkManager.Instance.localPlayerController.gameObject.name));
      bool flag = false;
      int hitSurfaceID = -1;
      if (!cancel)
      {
        this.previousPlayerHeldBy.twoHanded = false;
        Debug.DrawRay(this.previousPlayerHeldBy.gameplayCamera.transform.position + this.previousPlayerHeldBy.gameplayCamera.transform.right * -0.35f, this.previousPlayerHeldBy.gameplayCamera.transform.forward * 1.85f, Color.blue, 5f);
        this.objectsHitByShovel = Physics.SphereCastAll(this.previousPlayerHeldBy.gameplayCamera.transform.position + this.previousPlayerHeldBy.gameplayCamera.transform.right * -0.35f, 0.75f, this.previousPlayerHeldBy.gameplayCamera.transform.forward, 1.85f, this.shovelMask, QueryTriggerInteraction.Collide);
        ...
        Vector3 vector3_1 = this.previousPlayerHeldBy.gameplayCamera.transform.position;
        for (int index1 = 0; index1 < this.objectsHitByShovelList.Count; ++index1)
        {
              ...
              if (objectsHitByShovel.transform.TryGetComponent<IHittable>(out component))
              {
                ...
                if (!((UnityEngine.Object) objectsHitByShovel.transform == (UnityEngine.Object) this.previousPlayerHeldBy.transform))
                {
                  ...
                  if (!(objectsHitByShovel.point == Vector3.zero))
                  {
                    ...
                    if (Physics.Linecast(start, point, out local, roomMaskAndDefault))
                      continue;
                  }
                  flag = true;
          ...
          objectsHitByShovel = this.objectsHitByShovelList[index1];
          Vector3 point1 = objectsHitByShovel.point;
          objectsHitByShovel = this.objectsHitByShovelList[index1];
          Vector3 vector3_2 = objectsHitByShovel.normal * 0.01f;
          vector3_1 = point1 + vector3_2;
          flag = true;
          objectsHitByShovel = this.objectsHitByShovelList[index1];
          ...
          for (int index2 = 0; index2 < StartOfRound.Instance.footstepSurfaces.Length; ++index2)
          {
            ...
            {
              this.shovelAudio.PlayOneShot(StartOfRound.Instance.footstepSurfaces[index2].hitSurfaceSFX);
              WalkieTalkie.TransmitOneShotAudio(this.shovelAudio, StartOfRound.Instance.footstepSurfaces[index2].hitSurfaceSFX);
              hitSurfaceID = index2;
              break;
            }
          }
