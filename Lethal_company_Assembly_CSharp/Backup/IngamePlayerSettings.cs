...
public class IngamePlayerSettings : MonoBehaviour
{
  ...
  private void Awake()
  {
    if ((UnityEngine.Object) IngamePlayerSettings.Instance == (UnityEngine.Object) null)
    {
      ...
      if (!this.settings.playerHasFinishedSetup)
        return;
  ...
  {
    Debug.Log((object) "Changing gamma");
    float w = Mathf.Clamp((float) setTo * 0.05f, -0.85f, 2f);
    Debug.Log((object) string.Format("override float value: {0}", (object) overrideWithFloat));
    if ((double) overrideWithFloat != -500.0)
    {
      Debug.Log((object) "Overriding float");
      w = overrideWithFloat;
    }
    ...
    Debug.Log((object) "Gamma changed");
    this.unsavedSettings.gammaSetting = w;
    Debug.Log((object) string.Format("Set gamma to new value: {0}; {1}", (object) w, (object) component.gamma));
  }
