...
namespace DigitalRuby.ThunderAndLightning
{
  public static class PathGenerator
  {
    ...
    public static float SquareRoot(float x) => (float) Math.Sqrt((double) x);
    ...
    private static float Distance2D(ref Vector3 point1, ref Vector3 point2)
    {
      ...
      return PathGenerator.SquareRoot((float) (num1 * num1 + (double) num2 * (double) num2));
    }
    ...
    private static float Distance3D(ref Vector3 point1, ref Vector3 point2)
    {
      ...
      return PathGenerator.SquareRoot((float) (num1 * num1 + (double) num2 * (double) num2 + (double) num3 * (double) num3));
    }

    private static void GetCurvePoint2D(
      ref Vector3 start,
      ref Vector3 end,
      ref Vector3 ctr1,
      ref Vector3 ctr2,
    ...
    private static void GetCurvePoint3D(
      ref Vector3 start,
      ref Vector3 end,
      ref Vector3 ctr1,
      ref Vector3 ctr2,
    ...
    private static void CalculateNonuniformCatmullRom(
      float p1,
      float p2,
      float p3,
      float p4,
    ...
    private static void ClampSplineDistances(
      ref float distance1,
      ref float distance2,
      ref float distance3)
    {
      if ((double) distance2 < 9.9999997473787516E-05)
        distance2 = 1f;
      if ((double) distance1 < 9.9999997473787516E-05)
        distance1 = distance2;
      if ((double) distance3 >= 9.9999997473787516E-05)
        return;
      distance3 = distance2;
    }

    private static void GetSplinePoint2D(
      ref Vector3 point1,
      ref Vector3 point2,
      ref Vector3 point3,
      ref Vector3 point4,
    ...
    private static void GetSplinePoint3D(
      ref Vector3 point1,
      ref Vector3 point2,
      ref Vector3 point3,
      ref Vector3 point4,
      ...
      float startT)
    {
      ...
      {
        for (t = startT; (double) t <= 1.0; t += num)
        {
          Vector3 point;
      ...
      bool closePath)
    {
      if (points.Count < 4)
        return false;
      numberOfSegments = Math.Min(1024, Math.Max(numberOfSegments, 4));
      int num1 = closePath ? points.Count : points.Count - 1;
      int num2 = closePath ? 1 : 0;
      float num3 = 1f / (float) numberOfSegments * (float) num1;
      float num4 = 0.0f;
      for (int index1 = 0; index1 < num1; ++index1)
      {
        int index2 = index1 == 0 ? num1 - num2 : index1 - 1;
        int index3 = index1 + 1;
        int index4 = index1 + 2;
        if (closePath && index3 > num1 - 1)
          index3 -= num1;
        if (index4 > num1 - 1)
          index4 = closePath ? index4 - num1 : num1;
        ...
        if (PathGenerator.Is2D)
        {
          for (t = num4; (double) t <= 1.0; t += num3)
          {
      ...
      bool closePath)
    {
      ...
      for (int index1 = 0; index1 < num1; ++index1)
      {
        ...
        if (PathGenerator.Is2D)
        {
          ...
          for (float t = num3; (double) t <= 1.0; t += num4)
          {
            PathGenerator.GetSplinePoint2D(ref point1, ref point2, ref point3, ref point4, t, out point5);
            path.Add(point5);
          }
        ...
        {
          float num5 = Mathf.Clamp((float) (1.0 / ((double) PathGenerator.Distance3D(ref point3, ref point2) / (double) distancePerSegment)), 1f / 256f, 1f);
          for (float t = num3; (double) t <= 1.0; t += num5)
          {
            PathGenerator.GetSplinePoint3D(ref point1, ref point2, ref point3, ref point4, t, out point5);
            path.Add(point5);
          }
