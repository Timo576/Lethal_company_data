...
public class SandSpiderAI : EnemyAI
{
  ...
  public override void Start()
  {
    ...
    this.maxWebTrapsToPlace = UnityEngine.Random.Range(4, 7);
    ...
    else
    {
      ...
      switch (this.currentBehaviourStateIndex)
      {
        case 0:
          ...
          {
            this.timeSincePlacingWebTrap = UnityEngine.Random.Range(0.0f, 1.5f);
            this.AttemptPlaceWebTrap();
  ...
  private bool AttemptPlaceWebTrap()
  {
    ...
    this.ray = new Ray(this.abdomen.position, Vector3.Scale(UnityEngine.Random.onUnitSphere, new Vector3(1f, UnityEngine.Random.Range(0.6f, 1f), 1f)));
    if (Physics.Raycast(this.ray, out this.rayHit, 7f, StartOfRound.Instance.collidersAndRoomMask) && (double) this.rayHit.distance >= 1.5)
    {
      ...
      if (Physics.Raycast(this.abdomen.position, Vector3.down, out this.rayHit, 10f, StartOfRound.Instance.collidersAndRoomMask))
        this.SpawnWebTrapServerRpc(this.rayHit.point, point);
    }
    return false;
  }
