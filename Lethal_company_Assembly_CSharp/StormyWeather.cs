...
public class StormyWeather : MonoBehaviour
{
  ...
  private void OnEnable()
  {
    ...
    this.targetedThunderRandom = new System.Random();
  ...
  private void OnGlobalTimeSync()
  {
    ...
    this.seed = new System.Random((int) ((double) nearestTen * 10.0) + StartOfRound.Instance.randomMapSeed);
    this.timeAtLastStrike = TimeOfDay.Instance.globalTime;
  }
  ...
  private void DetermineNextStrikeInterval()
  {
    ...
    this.randomThunderTime += Mathf.Clamp((float) this.seed.Next(-5, 110) * 0.25f, 0.6f, 110f) / (float) Mathf.Clamp(TimeOfDay.Instance.currentWeatherVariable, 1, 100);
  }
  ...
  public void LightningStrike(Vector3 strikePosition, bool useTargetedObject)
  {
    ...
    {
      vector3 = index != 6 ? strikePosition + Vector3.up * 80f + new Vector3((float) random.Next(-32, 32), 0.0f, (float) random.Next(-32, 32)) : strikePosition + Vector3.up * 80f;
