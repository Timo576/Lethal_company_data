...
public class CrawlerAI : EnemyAI
{
  ...
  public override void Update()
  {
    ...
    switch (this.currentBehaviourStateIndex)
    {
      ...
      case 1:
        ...
        this.noticePlayerTimer += 0.075f;
        if ((double) this.noticePlayerTimer <= 1.5)
          break;
  ...
  private void CalculateAgentSpeed()
  {
    ...
    else
    {
      ...
      Animator creatureAnimator = this.creatureAnimator;
      Vector3 vector3 = Vector3.ClampMagnitude(this.transform.position - this.previousPosition, 1f);
      double num1 = (double) vector3.sqrMagnitude / ((double) Time.deltaTime / 2.25);
      creatureAnimator.SetFloat("speedMultiplier", (float) num1);
      vector3 = this.previousPosition - this.transform.position;
      float num2 = vector3.sqrMagnitude / (Time.deltaTime / 1.4f);
      if (this.IsOwner && (double) this.previousVelocity - (double) num2 > (double) Mathf.Clamp(num2 * 0.25f, 2f, 100f))
      {
        this.agent.speed = 0.0f;
        if (this.currentBehaviourStateIndex == 1)
        {
      ...
      else
      {
        ...
        this.agent.acceleration = Mathf.Clamp((float) (45.0 - (double) num2 * 12.0), 4f, 80f);
      }
    }
  }
  ...
  public override void HitEnemy(int force = 1, PlayerControllerB playerWhoHit = null, bool playHitSFX = false)
  {
    ...
    if (this.isEnemyDead)
      return;
    this.agent.speed = 2f;
    this.agentSpeedWithNegative = this.enemyHP != 1 ? UnityEngine.Random.Range(-0.25f, 2f) : UnityEngine.Random.Range(2f, 5f);
    if (!this.inSpecialAnimation)
