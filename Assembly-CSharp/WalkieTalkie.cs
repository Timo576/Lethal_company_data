...
public class WalkieTalkie : GrabbableObject
{
  ...
  public void SetPlayerSpeakingOnWalkieTalkieServerRpc(int playerId)
  {
    ...
    if (this.__rpc_exec_stage != NetworkBehaviour.__RpcExecStage.Server && (networkManager.IsClient || networkManager.IsHost))
    {
      ...
      FastBufferWriter bufferWriter = this.__beginSendServerRpc(64994802U, serverRpcParams, RpcDelivery.Reliable);
      BytePacker.WriteValueBitPacked(bufferWriter, playerId);
      this.__endSendServerRpc(ref bufferWriter, 64994802U, serverRpcParams, RpcDelivery.Reliable);
    }
  ...
  public void SetPlayerSpeakingOnWalkieTalkieClientRpc(int playerId)
  {
    ...
    if (this.__rpc_exec_stage != NetworkBehaviour.__RpcExecStage.Client && (networkManager.IsServer || networkManager.IsHost))
    {
      ...
      FastBufferWriter bufferWriter = this.__beginSendClientRpc(2961867446U, clientRpcParams, RpcDelivery.Reliable);
      BytePacker.WriteValueBitPacked(bufferWriter, playerId);
      this.__endSendClientRpc(ref bufferWriter, 2961867446U, clientRpcParams, RpcDelivery.Reliable);
    }
  ...
  public void UnsetPlayerSpeakingOnWalkieTalkieServerRpc(int playerId)
  {
    ...
    if (this.__rpc_exec_stage != NetworkBehaviour.__RpcExecStage.Server && (networkManager.IsClient || networkManager.IsHost))
    {
      ...
      ServerRpcParams serverRpcParams;
      FastBufferWriter bufferWriter = this.__beginSendServerRpc(2502573704U, serverRpcParams, RpcDelivery.Reliable);
      BytePacker.WriteValueBitPacked(bufferWriter, playerId);
      this.__endSendServerRpc(ref bufferWriter, 2502573704U, serverRpcParams, RpcDelivery.Reliable);
    }
    if (this.__rpc_exec_stage != NetworkBehaviour.__RpcExecStage.Server || !networkManager.IsServer && !networkManager.IsHost)
      return;
  ...
  public void UnsetPlayerSpeakingOnWalkieTalkieClientRpc(int playerId)
  {
    ...
    {
      ClientRpcParams clientRpcParams;
      FastBufferWriter bufferWriter = this.__beginSendClientRpc(3968582452U, clientRpcParams, RpcDelivery.Reliable);
      BytePacker.WriteValueBitPacked(bufferWriter, playerId);
      this.__endSendClientRpc(ref bufferWriter, 3968582452U, clientRpcParams, RpcDelivery.Reliable);
    }
    if (this.__rpc_exec_stage != NetworkBehaviour.__RpcExecStage.Client || !networkManager.IsClient && !networkManager.IsHost)
      return;
  ...
  private IEnumerator speakingIntoWalkieTalkieMode()
  {
    ...
    yield return (object) new WaitUntil(new Func<bool>(walkieTalkie.\u003CspeakingIntoWalkieTalkieMode\u003Eb__42_0));
        ...
        if ((UnityEngine.Object) WalkieTalkie.allWalkieTalkies[index1].playerHeldBy != (UnityEngine.Object) null && WalkieTalkie.allWalkieTalkies[index1].clientIsHoldingAndSpeakingIntoThis && WalkieTalkie.allWalkieTalkies[index1].isBeingUsed && this.isBeingUsed)
        {
          ...
          for (int index2 = WalkieTalkie.allWalkieTalkies[index1].audioSourcesToReplay.Count - 1; index2 >= 0; --index2)
          {
            ...
            if (!((UnityEngine.Object) key == (UnityEngine.Object) null))
            {
              ...
              {
                this.audioSourcesReceiving[key] = this.target.gameObject.AddComponent<AudioSource>();
              ...
              else
              {
                ...
                if ((double) num > (double) this.recordingRange + 7.0)
                {
                  ...
                  this.audioSourcesReceiving.Remove(key, ref audioSource);
        ...
        else if (this.talkiesSendingToThis.Contains(WalkieTalkie.allWalkieTalkies[index1]))
        {
          ...
          foreach (AudioSource key in WalkieTalkie.allWalkieTalkies[index1].audioSourcesToReplay)
          {
            for (int index3 = 0; index3 < WalkieTalkie.allWalkieTalkies.Count; ++index3)
            {
              ...
              {
                WalkieTalkie.allWalkieTalkies[index3].audioSourcesReceiving.Remove(key, ref audioSource);
                UnityEngine.Object.Destroy((UnityEngine.Object) audioSource);
              }
            }
          }
  ...
  internal static void InitializeRPCS_WalkieTalkie()
  {
    ...
    __nonvirtual (NetworkManager.__rpc_func_table.Add(64994802U, new NetworkManager.RpcReceiveHandler(WalkieTalkie.__rpc_handler_64994802)));
    // ISSUE: explicit non-virtual call
    __nonvirtual (NetworkManager.__rpc_func_table.Add(2961867446U, new NetworkManager.RpcReceiveHandler(WalkieTalkie.__rpc_handler_2961867446)));
    // ISSUE: explicit non-virtual call
    __nonvirtual (NetworkManager.__rpc_func_table.Add(2502573704U, new NetworkManager.RpcReceiveHandler(WalkieTalkie.__rpc_handler_2502573704)));
    // ISSUE: explicit non-virtual call
    __nonvirtual (NetworkManager.__rpc_func_table.Add(3968582452U, new NetworkManager.RpcReceiveHandler(WalkieTalkie.__rpc_handler_3968582452)));
  }

  private static void __rpc_handler_64994802(
    ...
    __RpcParams rpcParams)
  {
    ...
    else
    {
      ...
      ((WalkieTalkie) target).SetPlayerSpeakingOnWalkieTalkieServerRpc(playerId);
      target.__rpc_exec_stage = NetworkBehaviour.__RpcExecStage.None;
    }
  }

  private static void __rpc_handler_2961867446(
    NetworkBehaviour target,
    FastBufferReader reader,
    __RpcParams rpcParams)
  {
    ...
    ((WalkieTalkie) target).SetPlayerSpeakingOnWalkieTalkieClientRpc(playerId);
    target.__rpc_exec_stage = NetworkBehaviour.__RpcExecStage.None;
  }

  private static void __rpc_handler_2502573704(
    NetworkBehaviour target,
    FastBufferReader reader,
    __RpcParams rpcParams)
  {
    ...
    else
    {
      ...
      ByteUnpacker.ReadValueBitPacked(reader, out playerId);
      target.__rpc_exec_stage = NetworkBehaviour.__RpcExecStage.Server;
      ((WalkieTalkie) target).UnsetPlayerSpeakingOnWalkieTalkieServerRpc(playerId);
      target.__rpc_exec_stage = NetworkBehaviour.__RpcExecStage.None;
    }
  }

  private static void __rpc_handler_3968582452(
    NetworkBehaviour target,
    FastBufferReader reader,
    __RpcParams rpcParams)
  {
    ...
    ByteUnpacker.ReadValueBitPacked(reader, out playerId);
    target.__rpc_exec_stage = NetworkBehaviour.__RpcExecStage.Client;
    ((WalkieTalkie) target).UnsetPlayerSpeakingOnWalkieTalkieClientRpc(playerId);
    target.__rpc_exec_stage = NetworkBehaviour.__RpcExecStage.None;
  }

  protected internal override string __getTypeName() => nameof (WalkieTalkie);
}
