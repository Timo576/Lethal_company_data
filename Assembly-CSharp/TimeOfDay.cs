...
public class TimeOfDay : NetworkBehaviour
{
  ...
  {
    if (!this.IsServer || (double) this.RoundUpToNearestTen(this.globalTime) < (double) this.nextTimeSync)
      return;
    this.nextTimeSync = (float) this.RoundUpToNearestTen(this.globalTime + 10f);
  ...
  public void UpdateProfitQuotaCurrentTime()
  {
    ...
    {
      StartOfRound.Instance.deadlineMonitorBGImage.color = new Color(0.5294118f, 0.0196078438f, 0.8f, 1f);
      StartOfRound.Instance.profitQuotaMonitorBGImage.color = new Color(0.5294118f, 0.0196078438f, 0.8f, 1f);
