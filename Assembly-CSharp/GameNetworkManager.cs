...
public class GameNetworkManager : MonoBehaviour
{
  ...
  {
    DateTime dateTime1 = new DateTime(2023, 12, 11);
    DateTime dateTime2;
    try
    {
    ...
    {
      Debug.LogError((object) string.Format("Unable to get UTC time; defaulting to system date time; {0}", (object) ex));
      dateTime2 = DateTime.Today;
    }
    Debug.Log((object) string.Format("Returning week num: {0}", (object) (int) ((dateTime2 - dateTime1).TotalDays / 7.0)));
    return (int) ((dateTime2 - dateTime1).TotalDays / 7.0);
  }

  public string GetNameForWeekNumber(int overrideWeekNum = -1)
  {
    StringBuilder stringBuilder = new StringBuilder();
    System.Random random = overrideWeekNum == -1 ? new System.Random(this.GetWeekNumber()) : new System.Random(overrideWeekNum);
    List<char> list1 = ((IEnumerable<char>) "BCDFGKLMNPSTVZHRW".ToCharArray()).ToList<char>();
    List<char> list2 = ((IEnumerable<char>) "AEIOU".ToCharArray()).ToList<char>();
    int num1 = 0;
    int num2 = 1;
    char c = 'a';
    ...
    for (int index = 0; index < num3; ++index)
    {
      ...
      if (c == 'O' || c == 'E')
        --maxValue;
      if (random.Next(0, maxValue) > num2)
      {
        ...
        while (count > 0)
        {
          char ch = list1[random.Next(0, list1.Count)];
          if (list1.Count == 1 || num1 > 1 || random.Next(0, 100) < 33 || upper == 'Q' || upper == 'K' || upper == 'Q' && ch == 'B' || upper == 'H' && ch == 'P' || upper == 'I' && ch == 'W')
          {
            list1.Remove(ch);
            --count;
            num1 = 0;
        ...
        if (flag)
          list1 = ((IEnumerable<char>) "BCDFGJKLMNPSTVZHRW".ToCharArray()).ToList<char>();
        if ((int) c == (int) upper)
          ++num1;
        ++num2;
      }
      else
      {
        ...
        while (count > 0)
        {
          char ch = list2[random.Next(0, list2.Count)];
          if (list2.Count == 1 && upper == 'U' && ch == 'U' || upper == 'I' && ch == 'I' || upper == 'I' && ch == 'U' && index == 1)
          {
            list2.Remove(ch);
            --count;
            flag = true;
        ...
        num2 = 1;
        if (flag)
          list2 = ((IEnumerable<char>) "AEIOU".ToCharArray()).ToList<char>();
      }
      if (index != 0)
        c = char.ToLower(c);
      stringBuilder.Append(c);
    }
    return stringBuilder.ToString() + "-" + random.Next(1, 99).ToString();
  }
  ...
  private void Start()
  {
    ...
    {
      case -1:
        this.currentSaveFileName = "LCChallengeFile";
        break;
      case 0:
  ...
  private void SteamMatchmaking_OnLobbyCreated(Result result, Lobby lobby)
  {
    ...
    if (!string.IsNullOrEmpty(this.lobbyHostSettings.serverTag))
      lobby.SetData("tag", this.lobbyHostSettings.serverTag.ToLower());
    else
  ...
  public void SetLobbyJoinable(bool joinable)
  {
    ...
    else
    {
      ...
      if ((UnityEngine.Object) StartOfRound.Instance != (UnityEngine.Object) null && this.currentSaveFileName == "LCChallengeFile")
        GameNetworkManager.Instance.currentLobby.Value.SetData("chal", "t");
      else
        GameNetworkManager.Instance.currentLobby.Value.SetData("chal", "f");
    }
  }
  ...
  {
    if (this.isDisconnecting || (UnityEngine.Object) StartOfRound.Instance == (UnityEngine.Object) null)
      return;
  ...
  public void ResetUnlockablesListValues(bool onlyResetPrefabItems = false)
  {
    ...
    {
      if (!onlyResetPrefabItems || unlockables[index].spawnPrefab)
      {
  ...
  {
    if (StartOfRound.Instance.isChallengeFile && GameNetworkManager.Instance.gameHasStarted)
    {
      ...
      {
        Debug.Log((object) string.Format("Saved challenge score as {0}; total scrap in level: {1}", (object) StartOfRound.Instance.scrapCollectedLastRound, (object) RoundManager.Instance.totalScrapValueInLevel));
