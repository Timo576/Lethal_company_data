...
public class DoublewingAI : EnemyAI
{
  ...
  public void SetFlyDirection()
  {
    ...
    else
    {
      ...
      Vector3 vector3 = this.Body.position - this.previousPosition;
      float num = vector3.magnitude / Time.deltaTime;
      if (this.daytimeEnemyLeaving)
      {
        vector3 = this.Body.position - this.previousPosition;
      ...
      else if ((double) this.averageVelocity * (double) this.speedElevationMultiplier > 0.0 && this.Body.position - this.previousPosition != Vector3.zero)
        this.Body.rotation = Quaternion.Lerp(this.Body.rotation, Quaternion.LookRotation(this.Body.position - this.previousPosition, Vector3.up), 5f * Time.deltaTime);
