...
public class RoundManager : NetworkBehaviour
{
  ...
  {
    int num1 = (int) ((double) this.AnomalyRandom.Next(this.currentLevel.minScrap, this.currentLevel.maxScrap) * (double) this.scrapAmountMultiplier);
    if (StartOfRound.Instance.isChallengeFile)
    {
      int num2 = this.AnomalyRandom.Next(10, 30);
      num1 += num2;
      Debug.Log((object) string.Format("Anomaly random 0b: {0}", (object) num2));
    }
    ...
    {
      if (index == this.increasedScrapSpawnRateIndex)
        intList2.Add(100);
      else
    ...
    Debug.Log((object) string.Format("Number of scrap to spawn: {0}. minTotalScrapValue: {1}. Total value of items: {2}.", (object) ScrapToSpawn.Count, (object) this.currentLevel.minTotalScrapValue, (object) num3));
    ...
    for (int i = 0; i < ScrapToSpawn.Count; i++)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          num3 += intList1[intList1.Count - 1];
  ...
  public void SpawnMapObjects()
  {
    if (this.currentLevel.spawnableMapObjects.Length == 0)
      return;
    System.Random randomSeed = new System.Random(StartOfRound.Instance.randomMapSeed + 587);
    this.mapPropsContainer = GameObject.FindGameObjectWithTag("MapPropsContainer");
    ...
    {
      int num = (int) this.currentLevel.spawnableMapObjects[index1].numberToSpawn.Evaluate((float) randomSeed.NextDouble());
      if (this.increasedMapHazardSpawnRateIndex == index1)
      ...
      if (num > 0)
      {
        ...
        {
          RandomMapObject randomMapObject = randomMapObjectList[randomSeed.Next(0, randomMapObjectList.Count)];
          Vector3 inBoxPredictable = this.GetRandomNavMeshPositionInBoxPredictable(randomMapObject.transform.position, randomMapObject.spawnRange, randomSeed: randomSeed);
          GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(this.currentLevel.spawnableMapObjects[index1].prefabToSpawn, inBoxPredictable, Quaternion.identity, this.mapPropsContainer.transform);
          gameObject.transform.eulerAngles = !this.currentLevel.spawnableMapObjects[index1].spawnFacingAwayFromWall ? new Vector3(gameObject.transform.eulerAngles.x, (float) randomSeed.Next(0, 360), gameObject.transform.eulerAngles.z) : new Vector3(0.0f, this.YRotationThatFacesTheFarthestFromPosition(inBoxPredictable + Vector3.up * 0.2f), 0.0f);
          gameObject.GetComponent<NetworkObject>().Spawn(true);
        }
      }
    }
    ...
    int resolution = 6)
  {
    ...
    if (!this.hasInitializedLevelRandomSeed)
      return (float) UnityEngine.Random.Range(num1 - 15, num1 + 15);
    int num3 = this.AnomalyRandom.Next(num1 - 15, num1 + 15);
    Debug.Log((object) string.Format("Anomaly random yrotation farthest: {0}", (object) num3));
    return (float) num3;
  }
    ...
    int resolution = 6)
  {
    ...
    if (!this.hasInitializedLevelRandomSeed)
      return (float) UnityEngine.Random.Range(num1 - 15, num1 + 15);
    int num3 = this.AnomalyRandom.Next(num1 - 15, num1 + 15);
    Debug.Log((object) string.Format("Anomaly random yrotation nearest: {0}", (object) num3));
    return (float) num3;
  }
  ...
  private void SetChallengeFileRandomModifiers()
  {
    ...
    for (int index = 0; index < numArray.Length; ++index)
      numArray[index] = this.AnomalyRandom.Next(0, 100);
    if (numArray[0] < 45)
    {
      this.increasedInsideEnemySpawnRateIndex = this.AnomalyRandom.Next(0, this.currentLevel.Enemies.Count);
      if (this.currentLevel.Enemies[this.increasedInsideEnemySpawnRateIndex].enemyType.spawningDisabled)
        this.increasedInsideEnemySpawnRateIndex = this.AnomalyRandom.Next(0, this.currentLevel.Enemies.Count);
    }
    if (numArray[1] < 45)
      this.increasedOutsideEnemySpawnRateIndex = this.AnomalyRandom.Next(0, this.currentLevel.OutsideEnemies.Count);
    if (numArray[2] < 45)
      this.increasedMapHazardSpawnRateIndex = this.AnomalyRandom.Next(0, this.currentLevel.spawnableMapObjects.Length);
    if (numArray[3] < 45)
      this.increasedMapPropSpawnRateIndex = this.AnomalyRandom.Next(0, this.currentLevel.spawnableOutsideObjects.Length);
    if (numArray[4] >= 45)
      return;
    this.increasedScrapSpawnRateIndex = this.AnomalyRandom.Next(0, this.currentLevel.spawnableScrap.Count);
  }
  ...
  private IEnumerator LoadNewLevelWait(int randomSeed)
  {
    ...
    if (roundManager.currentLevel.spawnEnemiesAndScrap)
    {
      ...
      yield return (object) new WaitUntil(new Func<bool>(roundManager.\u003CLoadNewLevelWait\u003Eb__115_0));
      yield return (object) null;
      // ISSUE: reference to a compiler-generated method
      yield return (object) new WaitUntil(new Func<bool>(roundManager.\u003CLoadNewLevelWait\u003Eb__115_1));
  ...
  public void GenerateNewLevelClientRpc(int randomSeed, int levelID)
  {
    ...
    this.SetChallengeFileRandomModifiers();
    ...
    for (int index = 0; index < objectsOfType.Length; ++index)
    {
      if (despawnAllItems || !objectsOfType[index].isHeld && !objectsOfType[index].isInShipRoom || objectsOfType[index].deactivated || StartOfRound.Instance.allPlayersDead && objectsOfType[index].itemProperties.isScrap)
      {
        if (objectsOfType[index].isHeld && (UnityEngine.Object) objectsOfType[index].playerHeldBy != (UnityEngine.Object) null)
          objectsOfType[index].playerHeldBy.DropAllHeldItemsAndSync();
        NetworkObject component = objectsOfType[index].gameObject.GetComponent<NetworkObject>();
        ...
        {
          Debug.Log((object) ("Error/warning: prop '" + objectsOfType[index].gameObject.name + "' was not spawned or did not have a NetworkObject component! Skipped despawning and destroyed it instead."));
          UnityEngine.Object.Destroy((UnityEngine.Object) objectsOfType[index].gameObject);
        }
      }
      else
  ...
  private void SetToCurrentLevelWeather()
  {
    ...
    for (int index = 0; index < this.currentLevel.randomWeathers.Length; ++index)
    {
      ...
      {
        TimeOfDay.Instance.currentWeatherVariable = (float) this.currentLevel.randomWeathers[index].weatherVariable;
        TimeOfDay.Instance.currentWeatherVariable2 = (float) this.currentLevel.randomWeathers[index].weatherVariable2;
        if (StartOfRound.Instance.isChallengeFile)
        {
  ...
  {
    if (this.currentOutsideEnemyPower > this.currentMaxOutsidePower)
      return;
  ...
  public void SpawnDaytimeEnemiesOutside()
  {
    ...
    Debug.Log((object) string.Format("AnomalyRandom 2: {0}", (object) num2));
    GameObject[] gameObjectsWithTag = GameObject.FindGameObjectsWithTag("OutsideAINode");
    for (int index = 0; index < num2; ++index)
    {
  ...
  private GameObject SpawnRandomDaytimeEnemy(GameObject[] spawnPoints, float timeUpToCurrentHour)
  {
    ...
    for (int index = 0; index < this.currentLevel.DaytimeEnemies.Count; ++index)
    {
      ...
      if (enemyType.PowerLevel > this.currentLevel.maxDaytimeEnemyPowerCount - this.currentDaytimeEnemyPower || enemyType.numberSpawned >= this.currentLevel.DaytimeEnemies[index].enemyType.MaxCount || (double) enemyType.normalizedTimeInDayToLeave < (double) TimeOfDay.Instance.normalizedTimeOfDay || enemyType.spawningDisabled)
      {
    ...
    int randomWeightedIndex = this.GetRandomWeightedIndex(this.SpawnProbabilities.ToArray(), this.EnemySpawnRandom);
    ...
    Debug.Log((object) string.Format("Anomaly random 3: {0}, {1}, {2}", (object) positionInRadius.x, (object) positionInRadius.y, (object) positionInRadius.z));
    int index1 = 0;
    bool flag = false;
  ...
  private GameObject SpawnRandomOutsideEnemy(GameObject[] spawnPoints, float timeUpToCurrentHour)
  {
    ...
    for (int index = 0; index < this.currentLevel.OutsideEnemies.Count; ++index)
    {
      ...
      if (this.firstTimeSpawningOutsideEnemies)
        enemyType.numberSpawned = 0;
      if (enemyType.PowerLevel > this.currentMaxOutsidePower - this.currentOutsideEnemyPower || enemyType.numberSpawned >= enemyType.MaxCount || enemyType.spawningDisabled)
      {
        this.SpawnProbabilities.Add(0);
      }
      else
      {
        int num2 = this.increasedOutsideEnemySpawnRateIndex != index ? (!enemyType.useNumberSpawnedFalloff ? (int) ((double) this.currentLevel.OutsideEnemies[index].rarity * (double) enemyType.probabilityCurve.Evaluate(timeUpToCurrentHour / this.timeScript.totalTime)) : (int) ((double) this.currentLevel.OutsideEnemies[index].rarity * ((double) enemyType.probabilityCurve.Evaluate(timeUpToCurrentHour / this.timeScript.totalTime) * (double) enemyType.numberSpawnedFalloff.Evaluate((float) enemyType.numberSpawned / 10f)))) : 100;
        this.SpawnProbabilities.Add(num2);
        num1 += num2;
      }
    }
    ...
    if (num1 <= 0)
    {
      ...
      int currentMaxOutsidePower = this.currentMaxOutsidePower;
      return (GameObject) null;
    }
    int randomWeightedIndex = this.GetRandomWeightedIndex(this.SpawnProbabilities.ToArray(), this.EnemySpawnRandom);
    this.currentOutsideEnemyPower += this.currentLevel.OutsideEnemies[randomWeightedIndex].enemyType.PowerLevel;
    Vector3 positionInRadius = this.GetRandomNavMeshPositionInRadius(spawnPoints[this.AnomalyRandom.Next(0, spawnPoints.Length)].transform.position, 4f);
    Debug.Log((object) string.Format("Anomaly random 4: {0}, {1}, {2}", (object) positionInRadius.x, (object) positionInRadius.y, (object) positionInRadius.z));
    int index1 = 0;
    bool flag = false;
    for (int index2 = 0; index2 < spawnPoints.Length - 1; ++index2)
    {
  ...
  public void PlotOutEnemiesForNextHour()
  {
    ...
    if (this.currentEnemyPower >= this.currentMaxInsidePower)
    {
  ...
  private bool AssignRandomEnemyToVent(EnemyVent vent, float spawnTime)
  {
    ...
    for (int index = 0; index < this.currentLevel.Enemies.Count; ++index)
    {
      ...
      {
        int num2 = this.increasedInsideEnemySpawnRateIndex != index ? (!enemyType.useNumberSpawnedFalloff ? (int) ((double) this.currentLevel.Enemies[index].rarity * (double) enemyType.probabilityCurve.Evaluate(this.timeScript.normalizedTimeOfDay)) : (int) ((double) this.currentLevel.Enemies[index].rarity * ((double) enemyType.probabilityCurve.Evaluate(this.timeScript.normalizedTimeOfDay) * (double) enemyType.numberSpawnedFalloff.Evaluate((float) enemyType.numberSpawned / 10f)))) : 100;
        this.SpawnProbabilities.Add(num2);
  ...
  {
    return this.currentLevel.Enemies[enemyIndex].enemyType.spawningDisabled || this.currentLevel.Enemies[enemyIndex].enemyType.PowerLevel > this.currentMaxInsidePower - this.currentEnemyPower || this.currentLevel.Enemies[enemyIndex].enemyType.numberSpawned >= this.currentLevel.Enemies[enemyIndex].enemyType.MaxCount;
  }
  ...
  public void InitializeRandomNumberGenerators()
  {
    ...
    this.EnemySpawnRandom = new System.Random(this.playersManager.randomMapSeed - 40);
  ...
  {
    if (!this.IsServer)
      return (NetworkObjectReference) this.currentLevel.Enemies[0].enemyType.enemyPrefab.GetComponent<NetworkObject>();
  ...
  private void RefreshEnemiesList()
  {
    ...
    if (StartOfRound.Instance.isChallengeFile)
    {
      ...
      this.currentMaxInsidePower = this.currentLevel.maxEnemyPowerCount + random.Next(0, 8);
      this.currentMaxOutsidePower = this.currentLevel.maxOutsideEnemyPowerCount + random.Next(0, 8);
    }
    else
    {
      this.currentMaxInsidePower = this.currentLevel.maxEnemyPowerCount;
      this.currentMaxOutsidePower = this.currentLevel.maxOutsideEnemyPowerCount;
    }
  }
  ...
  private void SpawnOutsideHazards()
  {
    ...
    if (this.currentLevel.spawnableOutsideObjects != null)
    {
      ...
      {
        double time = randomSeed.NextDouble();
        int num2 = (int) this.currentLevel.spawnableOutsideObjects[index1].randomAmount.Evaluate((float) time);
        if (this.increasedMapPropSpawnRateIndex == index1)
          num2 += 12;
        if ((double) randomSeed.Next(0, 100) < 20.0)
          num2 *= 2;
        for (int index2 = 0; index2 < num2; ++index2)
        {
          ...
          if (!flag1)
          {
            ...
            ++num1;
            if (this.currentLevel.spawnableOutsideObjects[index1].spawnableObject.spawnFacingAwayFromWall)
            {
              gameObject.transform.eulerAngles = new Vector3(0.0f, this.YRotationThatFacesTheFarthestFromPosition(vector3 + Vector3.up * 0.2f), 0.0f);
            }
            else
            {
              int y = randomSeed.Next(0, 360);
              gameObject.transform.eulerAngles = new Vector3(gameObject.transform.eulerAngles.x, (float) y, gameObject.transform.eulerAngles.z);
            }
  ...
  private void SetLockedDoors(Vector3 mainEntrancePosition)
  {
    ...
    if (!this.IsServer)
      return;
    for (int index = 0; index < num2; ++index)
      UnityEngine.Object.Instantiate<GameObject>(this.keyPrefab, this.GetRandomNavMeshPositionInBoxPredictable(this.insideAINodes[this.AnomalyRandom.Next(0, this.insideAINodes.Length)].transform.position, 8f, this.navHit, this.AnomalyRandom), Quaternion.identity, this.spawnedScrapContainer).GetComponent<NetworkObject>().Spawn();
  }
    ...
    int layerMask = -1)
  {
    ...
    Vector3 vector3 = (new Vector3((float) x, (float) y2, (float) z) + pos) with
    {
      y = y1
    };
    float num3 = Vector3.Distance(pos, vector3);
    return NavMesh.SamplePosition(vector3, out navHit, num3 + 2f, layerMask) ? navHit.position : pos;
  }
  ...
  {
    radius *= 2f;
    float num1 = this.RandomFloatWithinRadius(minRadius, radius, randomGen);
    float num2 = this.RandomFloatWithinRadius(minRadius, radius, randomGen);
    float num3 = this.RandomFloatWithinRadius(minRadius, radius, randomGen);
    return new Vector3(pos.x + num1, pos.y + num2, pos.z + num3);
  }
