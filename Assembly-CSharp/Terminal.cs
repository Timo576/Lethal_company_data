...
public class Terminal : NetworkBehaviour
{
  ...
  public void RunTerminalEvents(TerminalNode node)
  {
    ...
    if (!this.IsServer || !(node.terminalEvent == "ejectPlayers") || StartOfRound.Instance.isChallengeFile || !StartOfRound.Instance.inShipPhase || StartOfRound.Instance.firingPlayersCutsceneRunning)
      return;
  ...
  public void BuyItemsServerRpc(int[] boughtItems, int newGroupCredits, int numItemsInShip)
  {
    ...
    if (this.__rpc_exec_stage != NetworkBehaviour.__RpcExecStage.Server || !networkManager.IsServer && !networkManager.IsHost || boughtItems.Length > 12 || newGroupCredits > this.groupCredits)
      return;
  ...
  private void Start()
  {
    ...
    if (this.IsServer)
    {
      ...
      if (StartOfRound.Instance.isChallengeFile)
      {
        ...
        this.groupCredits = random.Next(0, 100) >= 50 ? random.Next(20, 300) : random.Next(20, 900);
      }
      else
      {
        num = ES3.Load<int>("Reimburse", GameNetworkManager.Instance.currentSaveFileName, 0);
      ...
      this.startingCreditsAmount = this.groupCredits;
    ...
    int[] storyLogs = null)
  {
    ...
    if (this.__rpc_exec_stage != NetworkBehaviour.__RpcExecStage.Client || !networkManager.IsClient && !networkManager.IsHost || this.syncedTerminalValues)
      return;
    this.syncedTerminalValues = true;
    this.startingCreditsAmount = newGroupCredits;
